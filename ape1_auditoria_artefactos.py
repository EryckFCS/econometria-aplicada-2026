...
# Cargar datos
df = pd.read_csv("/home/user/workspace/audit/panel_raw.csv")
manifest_df = pd.read_csv("/home/user/workspace/audit/manifest_fuentes_raw.csv")
with open("/home/user/workspace/audit/series_catalog.pkl", "rb") as f:
...
if __name__ == "__main__":
    ...
    dic.to_csv("/home/user/workspace/audit/diccionario_variables.csv", index=False, encoding="utf-8-sig")
    print(f"  ✓ diccionario_variables.csv ({len(dic)} filas)")

    # 3. Cumplimiento consigna
    cum = build_cumplimiento()
    cum.to_csv("/home/user/workspace/audit/matriz_cumplimiento_consigna.csv", index=False, encoding="utf-8-sig")
    print(f"  ✓ matriz_cumplimiento_consigna.csv ({len(cum)} filas)")

    # 4. Manifest enriquecido
    man = build_manifest()
    man.to_csv("/home/user/workspace/audit/manifest_fuentes.csv", index=False, encoding="utf-8-sig")
    print(f"  ✓ manifest_fuentes.csv ({len(man)} filas)")

    # 5. Redundancia
    red = build_redundancia()
    red.to_csv("/home/user/workspace/audit/auditoria_redundancia.csv", index=False, encoding="utf-8-sig")
    print(f"  ✓ auditoria_redundancia.csv ({len(red)} filas)")

    # 6. Integridad
    integ = build_integridad()
    integ.to_csv("/home/user/workspace/audit/auditoria_integridad.csv", index=False, encoding="utf-8-sig")
    print(f"  ✓ auditoria_integridad.csv ({len(integ)} filas)")

    # 7. Variables no resueltas
    with open("/home/user/workspace/audit/variables_no_resueltas.md", "w", encoding="utf-8") as f:
        f.write(NO_RESUELTAS_MD)
    print("  ✓ variables_no_resueltas.md")

    # 8. README
    with open("/home/user/workspace/audit/README_AUDITORIA.md", "w", encoding="utf-8") as f:
        f.write(README_MD)
    print("  ✓ README_AUDITORIA.md")

    # Propuesta panel_modelo (artefacto bonus)
    prop = build_propuesta()
    prop.to_csv("/home/user/workspace/audit/propuesta_panel_modelo.csv", index=False, encoding="utf-8-sig")
    print(f"  ✓ propuesta_panel_modelo.csv ({len(prop)} filas)")

    print("\n✓ Todos los artefactos generados en /home/user/workspace/audit/")

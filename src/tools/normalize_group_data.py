...
def main():
    ...
    df5 = std_erik(df_rigor, {"Impuesto medioambiental ": "impuesto_medioambiental_moreno"})

    # Unificar todas
    erik_final = df1.merge(df2, on=["iso2", "year"], how="outer") \
    ...
    df[["iso2", "year", "calidad_institucional", "gobernanza", "informalidad"]].to_csv(OUTPUT_DIR / "marvin_v1_std.csv", index=False)
    ...
    df_long[["iso2", "year", "tecnologia"]].to_csv(OUTPUT_DIR / "marvin_v2_std.csv", index=False)
    ...
    cols = ["iso2", "year", "indice_riesgo_climatico", "PIBpc", "renta_rec_nat", "impuestos_ambientales", "protec_der_lab"]
    ...
    df = df.rename(columns={"TIME_PERIOD": "year", "OBS_VALUE": "innovacion_financiera_access"})
    df["year"] = pd.to_numeric(df["year"], errors="coerce").dropna().astype(int)
    df = df.dropna(subset=["iso2", "year", "innovacion_financiera_access"])
    df = df.drop_duplicates(subset=["iso2", "year"], keep="first")
    df[["iso2", "year", "innovacion_financiera_access"]].to_csv(OUTPUT_DIR / "abel_v3_std.csv", index=False)
    ...
    df = df.rename(columns={"Año": "year", "pob": "poblacion", "incert_energ": "incertidumbre_energetica"})
    df[["iso2", "year", "poblacion", "incertidumbre_energetica"]].to_csv(OUTPUT_DIR / "helen_std.csv", index=False)

...
def task_merge_api_local():
    ...
    catalog_full = series_equipo + [s for s in series_ape1 if s["nombre_raw"] not in nombres_grupo]
    ...
    profile = profile.__class__(
        ...
        source_priority=("world_bank", "local_file"), # Buscar en API primero, luego local
        allow_partial=True,
    ...
    exporter.to_academic_excel(df, catalog_full, filename="MASTER_FINAL_CONSOLIDADO_API_LOCAL.xlsx")

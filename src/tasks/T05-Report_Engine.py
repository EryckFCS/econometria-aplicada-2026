...
def run_quarto_render(target_format: str = "all"):
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info(f"✅ Renderizado completado con éxito.")
            logger.info(f"📂 Archivos generados en: {OUTPUT_DIR}")
        else:
            logger.error(f"❌ Error en el renderizado de Quarto:")
            logger.error(result.stderr)
            
    except FileNotFoundError:
        logger.error("❌ 'quarto' no está instalado en el sistema o no está en el PATH.")

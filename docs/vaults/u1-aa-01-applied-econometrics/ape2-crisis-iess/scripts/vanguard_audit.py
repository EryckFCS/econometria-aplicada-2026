...
def main():
    ...
    df_base = pd.read_parquet(DATA_PATH / "iess_quarterly.parquet").reset_index(
        drop=True
    )
    ...
    for r in results:
        bic = (
            f"{r.get('bic', 0):10.2f}"
            if "bic" in r and isinstance(r.get("bic"), (int, float))
            else "N/A"
        )
        pval = (
            f"{r.get('p_val', 0):10.4f}"
            if "p_val" in r and isinstance(r.get("p_val"), (int, float))
            else "N/A"
        )

...
sel_res = ardl_select_order(
    df[dep_var], maxlag=4, exog=df[indep_vars], maxorder=2, ic="bic", trend="c"
)
...
try:
    ...
    exog_cols = (
        ["y_l1"]
        + [f"{v}_l1" for v in selected_exog]
        + [f"d_{v}" for v in selected_exog]
    )
    ...
    crit_vals = {"10%": (2.72, 3.77), "5%": (3.23, 4.35), "1%": (4.29, 5.61)}
...
try:
    ...
    print(
        f"Ljung-Box (Autocorr lag 4): {lb['lb_stat'].values[0]:.4f} (p={lb['lb_pvalue'].values[0]:.4f})"
    )
    print(
        "Note: High autocorrelation is expected due to the interpolation process (induced smoothness)."
    )
